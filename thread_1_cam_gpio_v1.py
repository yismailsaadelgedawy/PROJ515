...
from sort import *
# Import the MaintenanceHandler from upload_utils along with existing imports
from upload_utils import update_hive_data, send_alert, MaintenanceHandler
...
# GPIO pin configuration for maintenance mode
...
# Function to upload data to Firebase when maintenance mode is activated
def firebase_flush_callback(bee_stats):
    """
    Immediately push all current data to Firebase when maintenance mode is activated
    
    Args:
        bee_stats: The BeeStats instance with the current monitoring data
    ...
    try:
        ...
        # Push activity data to Firebase
        success = update_hive_data(
            USER_ID,
            HIVE_ID,
            activity=bee_stats.activity_level,
            additional_data={
                'wasps_detected': bee_stats.potential_wasps > 0,
                'bee_count': len(bee_stats.unique_bees),
                'entering': bee_stats.entering_hive,
                'leaving': bee_stats.leaving_hive
            }
        )
...
def detect_and_monitor():
    ...
    maintenance.set_callback(lambda: firebase_flush_callback(bee_stats))
    ...
    try:
        while running:
            ...
            if not hasattr(maintenance, 'gpio_initialized') or not maintenance.gpio_initialized:
                ...
                print(f"Continuing monitoring in 5 seconds...")
        ...
        try:
            maintenance.cleanup()
            print("Maintenance handler resources cleaned up")
        except Exception as e:
